...
#if !defined(CHECKSUM_CRC32C) && !defined(CHECKSUM_CRC32C_HARDWARE)
 ...
 *
 * TODO: use glibc memcpy when size is large enough
 *
...
inline uint32_t normal_bswap(uint32_t v) {
  return (v << 24) | (v>>24) | (v<<8 & 0x00ff0000) | (v>>8 & 0x0000ff00);
}

inline uint64_t normal_bswap64(uint64_t v) {
  return (v << 56) |
 ...
 */
inline uint32_t crc_val(uint32_t crc) {
  return ~crc;
}
 ...
 */
inline uint32_t crc32_sb8(uint32_t crc, const uint8_t *buf, size_t length) {
  uint32_t running_length = ((length)/8)*8;
  uint32_t end_bytes = length - running_length;
  uint32_t li;
  for (li=0; li < running_length/8; li++) {
...
#ifdef CHECKSUM_CRC32C_HARDWARE
 ...
 */
static uint32_t crc32_sb8(uint32_t crc, const uint8_t* data, size_t length) {
...
typedef checksum_t (*ChecksumFunction) (checksum_t old, void * buff, uint32_t len);

inline void checksum_update(checksum_t & sum, const void * buff, uint32_t len) {
  //fprintf(stderr, "checksum_update: %016lx %u\n", (size_t)buff, len);
  sum = crc32_sb8(sum, (const uint8_t*)buff, len);
}

/**
 * Initialize a CRC
 */
inline checksum_t checksum_init() {
  return crc_init();
}
 ...
 * Extract the final result of a CRC
 */
inline checksum_t checksum_val(checksum_t crc) {
  return crc_val(crc);
}


/**
 * hadoop VInt writable stuff
 */

const uint32_t MaxVUInt32Length = 5;
const uint64_t MaxVUInt64Length = 9;
const uint16_t IFileEndMarker = 0xffff;
const char VIntMinusOneChar = 0xff;

// VInt format
// | -128 -127 ... -121 | -120 ... -113 | -112 ... 127
// |  minus             | plus          | normal value

inline void WriteVUInt32(char *&position, uint32_t value) {
  char * pos = position;
  if (value<128) {
    *(pos++) = (unsigned)value;
  }
...
inline uint32_t ReadVUInt32(char * &pos) {
  assert(*pos>=-116);
  if (*pos>=-112) {
    return *pos++;
  }
  char * end = pos + (-111 - *pos);
  uint32_t value = 0;
  while (++pos < end) {
...
inline uint32_t GetVUInt32Len(uint32_t value) {
  if (value<128) {
    return 1;
  } else if (value<(1<<8)) {
    return 2;
...
inline uint32_t GetVUInt32BufferLen(char * pos) {
  assert(*pos>=-116);
  if (*pos>=-112) {
    return 1;
  }
...
inline uint32_t ReadVUInt32_opt(char *&pos) {
  assert(*pos>=-116);
  if (*pos>=-112) {
    return *pos++;
  }
  ...
  value = bswap(value) & (1U << (len*8) - 1U);
  pos += len;
  return value;
}

/**
 * big-endian 4 byte int format
 */
inline void WriteUInt32(char * buff, uint32_t value) {
  *(uint32_t*)buff = bswap(value);
}

inline uint32_t ReadUInt32(char * buff) {
  return bswap(*(uint32_t*)buff);
}
